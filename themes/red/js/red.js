	...
	if(data.id != current_user_id){
		...
		if(data.action == 'message'){
			...
			if(currentModal != 'messages'){
				var a = parseInt($('.has-unread-msg').first().text()) +1;
				$('.has-unread-msg').text(a);
				$('.has-unread-msg').show();
				var onclickC = data.id+','+data.name+',"'+data.icon+'"';
				var onclick = "openModal('messages','"+onclickC+"')";
				sendToast('New private message',data.name+' just sended you a private message',"'"+data.icon+"'",4500,onclick);
			}


	    	var messageVal = data.clean_msg;
			var msgTime = formatAMPM(new Date);

			if ($('[data-creator-chat='+data.id+']').length){
				if(data.type != 'text'){
					$('[data-chat-lastm='+data.id+']').html('<svg class="icon icon-picture" style="width:15px;height:15px"><use xlink:href="#icons-picture"></use></svg>');
				} else {
					$('[data-chat-lastm='+data.id+']').text(messageVal);
				}
			} else {

				$('#new-chat').show();	

				$('#new-chat').find('.avatar__img').css('background-image', 'url('+data.icon+')');
				$('#new-chat').find('.displayname__name--text-overflow').text(data.name);
				$('#new-chat').find('.personal-notifications-message-item-body').attr('data-chat-lastm',data.id);
				$('#new-chat').find('.personal-notifications-message-item-body').attr('data-chat-lastm',data.id);
				$('#new-chat').find('.bDc__new').attr('data-chat-unread-count',data.id);

				$('#new-chat').attr('data-creator-chat',data.id);
				$('#new-chat').attr('onClick','loadConversation('+data.id+',0)');
			...
			reloadConversation(data.id);
		
			var a = parseInt($('[data-chat-unread-count='+data.id+']').text());
			var cc = a+1;
			$('[data-chat-unread-count='+data.id+']').text(cc);
			$('[data-creator-chat='+data.id+']').attr('data-unread',cc);
			$('[data-chat-unread-count='+data.id+']').show();
...
$("#upload-area").dmUploader({
    ...
    onNewFile: function(id, file){
    	...
    	if(upType == 25){ //DOCUMENTS   	
			createDocumentPreview(file,fileUrl,documentId);
		}		
		...
		if(upType == 20){
			...
			$('#publishPostBtn').attr('disabled',true); 
		}

		if(upType == 5){
			$('#myProfileAvatar').css('background-image','url('+fileUrl+')');
			$('#myProfileAvatar').addClass('uploadingGray');
		}
	...
	if(upType == 20){
	    ...
	    upphotos[0] = file;
	    upphotos[0]['tmp_id'] = id;
	}
    var photoPath = file.path;


    if(upType == 25){ //DOCUMENTS
    	$(documentId).removeClass('tat--with-image');
    	$(documentId).addClass('tat--with-image-complete');
    	$(documentId).find('.tat__content').hide();
    	var d = documentId.replace("#", "");
    	becomeCreator[d] = photoPath;
    }
    ...
    if(upType == 5){  
		$.ajax({
		  ...
		  success: function(t) {
			$('#myProfileAvatar').removeClass('uploadingGray');
			$('.headerAvatar').css('background-image','url('+photoPath+')');
		  }
		});    	
    }
    ...
    if(upType == 20){
		...
		$('#publishPostBtn').attr('disabled',false);
    }
...
function selectPayout(method,email,help='',time='',val=''){
	wmethod = method;

	$('[data-payout-tab]').removeClass('radio-pill-controls__item--active');
	$('[data-payout-tab="'+method+'"]').addClass('radio-pill-controls__item--active');

	$('#payoutEmail').hide();
	$('#payoutDetails').hide();
	$('#payoutEmailInput').val('');
	$('#payoutDetailsInput').val('');
	if(email == 1){
		if(val != ''){
			$('#payoutEmailInput').val(val);
			$('.textareaplaceholder').hide();
		}
		$('#payoutEmail').show();
		$('#payoutEmailInput').attr('placeholder',help);
		if(time != ''){
			$('.payoutDetailsHelp').html('');
			$('.payoutDetailsHelp').append(time);
		}		
		$('#payoutEmailInput').attr('onchange',"updateCreatorPayoutMethod('"+method+"','email')");
		$("#requestPayoutBtn").attr("onclick","withdrawNow('email')");
	} else {
		if(val != ''){
			var v = val;
			$('#payoutDetailsInput').val(v.replace(/<br *\/?>/gi, '\n'));
			$('.textareaplaceholder').hide();
		} else {
			$('.textareaplaceholder').show();
		}		
		$('#payoutDetails').show();
		$('.payoutDetailsHelp').html(help);
		if(time != ''){
			$('.payoutDetailsHelp').append('<br><br>'+time);
		}
		$('#payoutDetailsInput').attr('onchange',"updateCreatorPayoutMethod('"+method+"','details')");
		$("#requestPayoutBtn").attr("onclick","withdrawNow('details')");
	}
	$("#requestPayoutBtn").removeClass("disabled");
}
...
function withdrawNow(details){
	...
	e = amount;
	if(details == 'email'){
		var p = $('#payoutEmailInput').val();
	} else {
		var p = $('#payoutDetailsInput').val();
	}
	var info = $('#payoutNote').val();
	if(p == ''){
		sendToast('No payout method','Make sure to configure correctly your selected payout method',"'"+user_info.profile_photo+"'",4500);
		return false;		
	}

	if(user_info.credits < amount){
		sendToast('Not enought balance','You dont have enought balance in your account for request a '+amount+' '+plugins['settings']['currency']+' payment',"'"+user_info.profile_photo+"'",4500);
		return false;		
	}

	if (user_info.credits < plugins['withdrawal']['minRequired']) {
		sendToast('Not enought balance',site_lang[585]['text'],"'"+user_info.profile_photo+"'",4500);
		return false;	    
	}  else {
...
function current_chat(user) {
    ...
    channel.bind(eventBind, function(data) {
		...
		var msgTime = formatAMPM(new Date);
		var newMessage = `
		<div class="base-message-wrapper base-message-wrapper--position--left msg-you">
			<div class="base-message-wrapper__content">
			    ...
			    <div class="base-message-wrapper__body-wrapper">  
		            <div class="base-message counterpart-base-message counterpart-text-message">
		                `+messageVal+`   
		...
		if(data.id == current_user_id){
			...
			var addMessageToArray = loadedChats[data.id]['chat']+newMessage;
			loadedChats[data.id]['chat'] = addMessageToArray;
...
function selectEmoji(emoji){
	var currentMessage = $('.emoji-input').html();
	var emojiHtml = '<img src="'+site_config.site_url+emoji+'" class="smile smile--size--small">';
	$('.emoji-input').html(currentMessage+''+emojiHtml);
	showEmoji();
}
