...
(function() {
    ...
    this.Story = function(data) {
        ...
        var baseHTML = '<div class="storytime" style="opacity: 0; display: none;">' +
            ...
            '</div>'+             
			'<a href="javascript:;" class="story-arrow left" onclick="socialStory.prev();"></a><a href="javascript:;" class="story-arrow right" onclick="socialStory.next();"></a>' +
				'<div class="story-nav">' +
					'<div class="story-nav-left">'+
                    '<div class="paused"><span>'+site_lang[614]['text']+'</span></div>'+
                    '<div class="story-icon vivify popIn box-shadow" onclick="socialStory.close(); goToProfile(event,'+storyUser+');"></div> <span class="story-text text-shadow" onclick="socialStory.close(); goToProfile(event,'+storyUser+');" style="cursor:pointer;"></span><span class="story-date text-shadow"></span></div>' +
                    '<div class="story-nav-right">'+
                        '<div class="storySettings"><svg class="icon icon-settings R__actions-settings-icon R__actions-settings-icon--padded"><use xlink:href="#icons-settings"></use></svg></div>'+                   
    ...
    function launch() {
        ...
        storyUsername = defaults.playlist[start].title;
        ...
        if(defaults.playlist[start].stype == 'video'){
            storyContainer.innerHTML = '<video class="story-next videoStory" src=""  style="border-radius:2px" playsinline></video><div class="leftSection" onclick="socialStory.prev();"></div><div class="rightSection" onclick="socialStory.next();"></div>';
            ...
            storySpinner.style.display = 'none';
            thisTimeline.style.width = '0%';
            storyContainer.innerHTML = '<img class="story-next videoStory" src=""  style="border-radius:2px" /><div class="leftSection" onclick="socialStory.prev();"></div><div class="rightSection" onclick="socialStory.next();"></div>';  
        ...
        if(storyCredits > 0 && storyPurchased == 0 && myStory == 0 && !storyAdmin){         
            ...
            $('.story-window').append('<div class="story-blur box-shadow" style="background:#d60908" onclick="socialStory.close();goToProfile(event,'+storyUser+',`subscribe`);"><span>Suscribe to '+storyUsername+' for unlock</span></div>');
            $('[data-story-chat-input]').hide();
            /*
...
function openStoryProfile(element,uid,storyDay,openAdmin=false){
    console.log('launch story');
    storyPage = 'discover';
    viewingStory = $(element).attr('data-current-story-position');
    if(!isStoryLoading){
