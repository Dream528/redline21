...
function getDASelectedCol($col,$table,$filter) {
	global $mysqli;
	$result = [];
	$query = $mysqli->query("SELECT $col FROM $table $filter");
	$result = $query->fetch_assoc();
	return $result;
}
...
function getUserStories($name,$photo,$filter='',$order='',$limit='',$storyDay=0){
	global $mysqli,$sm;
	$result = array();
	$i = 0;
	...
	if($query->num_rows >= 1) {
		while($row = $query->fetch_assoc()){
			if(isset($sm['user'])){
				$checkSub = isSub($sm['user']['id'],$row['uid']);
...
function profilePhoto($uid,$big=0) {
	...
	if($photo->num_rows == 1) {
		...
		$profile_photo = randomPhoto($uid);
	}
	return $profile_photo;
}

function profileCover($uid) {
	global $mysqli,$sm;
	$uid = secureEncode($uid);
	$profile_photo = array();
	$photo = $mysqli->query("SELECT photo,thumb,video FROM users_photos where u_id = '".$uid."' and cover = 1 order by id asc LIMIT 1");
	if(isset($photo->num_rows) && !empty($photo->num_rows)){
		$profile = $photo->fetch_object();	
		$profile_photo['media'] = $profile->photo;
		$profile_photo['video'] = $profile->video;
		$profile_photo['poster'] = '';
	} else {
		$profile_photo['media'] = randomVideo($uid,1);
		$profile_photo['video'] = 0;
		if (strpos($profile_photo['media'], 'mp4') !== false) {
			$profile_photo['video'] = 1;
			$profile_photo['poster'] = getData('users_photos','thumb','WHERE photo = "'.$profile_photo['media'].'"');
		}
	}
	return $profile_photo;
}

function profileDiscover($uid) {
	global $mysqli,$sm;
	$uid = secureEncode($uid);
	$profile_photo = array();
	$photo = $mysqli->query("SELECT photo,thumb,video FROM users_photos where u_id = '".$uid."' and discover = 1 order by id asc LIMIT 1");
	if(isset($photo->num_rows) && !empty($photo->num_rows)){
		$profile = $photo->fetch_object();	
		$profile_photo['media'] = $profile->photo;
		$profile_photo['video'] = $profile->video;
...
function randomPhoto($uid,$big=0) {
	global $mysqli,$sm;
	$uid = secureEncode($uid);
	$photos = array();
...
function randomVideo($uid) {
	global $mysqli,$sm;
	$uid = secureEncode($uid);
	$photos = array();
	$photo = $mysqli->query("SELECT photo,thumb FROM users_photos where u_id = ".$uid." and approved = 1 and video = 1 ORDER BY rand() LIMIT 1");
	if(isset($photo->num_rows) && !empty($photo->num_rows)){
		while($profile = $photo->fetch_object()){	
			$photos[] = $profile->photo;	
		}
	} else {
		$photos[] = randomPhoto($uid,1);
	}
	return $photos[0];
}

function profileDiscoverPhoto($uid,$big=0) {
	global $mysqli,$sm;
	$uid = secureEncode($uid);
	$photos = array();
	$photo = $mysqli->query("SELECT photo,thumb FROM users_photos where u_id = ".$uid." and discover = 1 ORDER BY rand() LIMIT 1");
	if(isset($photo->num_rows) && !empty($photo->num_rows)){
		while($profile = $photo->fetch_object()){	
			if($big == 1){
				$photos[] = $profile->photo;
...
function getUserInfo($uid,$value=0) {
	...
	$current_user['creator'] = $u->creator;
	...
	$current_user['creator_pending'] = getData('users_verification','status','WHERE uid = '.$u->id);
	...
	if(empty($u->username)){
		$current_user['username'] = strtolower($u->name);	
	} else {
		$current_user['username'] = strtolower($u->username);	
	}	
	...
	$current_user['instagram'] = $u->instagram;		
	$current_user['twitter'] = $u->twitter;
	$current_user['snapchat'] = $u->snapchat;
	$current_user['tiktok'] = $u->tiktok;	
	$current_user['telegram'] = $u->telegram;			
...
function welcomeMailNotification($user_name,$user_email,$user_password){
	...
	try {	
		...
		$mail->SMTPSecure = $sm['plugins']['email']['protection'];	
...
function subscriptionsPrice() {
	global $mysqli,$sm;
	$result=array();
	$query= $mysqli->query("SELECT * FROM config_subscriptions_prices order by days asc");
	while($row = $query->fetch_assoc()){
		$result[] = $row;
	}	
	return $result;
...
function isVideo($url) {
    $video_extensions = array('mp4', 'webm', 'avi', 'mov', 'mkv'); // Add more video extensions as needed
...
function date_dropdown_landing($d=0,$m=0,$y=0){
	...
	if($y==1){
		for ($year = date("Y")-$year_limit; $year >= (1960); $year--) {
...
function googleconnect($id,$name,$email,$gender,$photo,$location){
	...
	if ($check != 'noData'){	
		...
		if($checkUsername == 1){
			$username = $username.rand(0,10421);				
		}
		...
		$query = "INSERT INTO users (name,email,pass,age,gender,city,country,lat,lng,looking,lang,join_date,bio,s_gender,s_age,credits,online_day,password,ip,last_access,username,join_date_time,verified)
			VALUES ('".$name."', '".$email."','".$pswd."','23','".$gender."','".$city."','".$country."','".$lat."','".$lng."','".$looking."','".$lang."','".$date."','".$bio."','".$looking."','".$sage."',0,0,0,'".$ip."','".time()."','".$username."','".time()."',1)";					
		if ($mysqli->query($query) === TRUE) {
			$last_id = $mysqli->insert_id;
			$_SESSION['user'] = $last_id;
			$mysqli->query("INSERT INTO users_videocall (u_id) VALUES ('".$last_id."')");
...
function getLandingPage($page_url='',$customLanding='') {
    ...
    if($customLanding != ''){
    	...
    	$page = './themes/red/layout/' . $page_url . '.phtml';	
    }
    $page_content = '';
    ob_start();
    include($page);
	...
	if($query && $query->num_rows > 0){ 
		while($q = $query->fetch_object()){	
			...
			if (!in_array($id, $result)){
				...
				echo
				'<div class="bp__item">
	                <span class="avatar" style="--avatar-size: 80px;">
	                    <span class="avatar__story-border avatar__story-border--active"></span>
	                    <span class="avatar__img" style="background-image: url(https://video-thumbs.my.club/public/245885/c559ee921d5f3bddfed91479b9780898/240p/thumb-0004.jpg);"></span>
	                </span>
	            </div>				
				';
...
function redStories($looking,$filter='',$custom=''){
	global $sm,$mysqli;
	...
	if($custom == 'favs'){
		$query = $mysqli->query("
	        ...
	        SELECT id,uid,storyTime,story,storyType
	        FROM users_story s
	...
	if($query && $query->num_rows > 0){ 
		while($q = $query->fetch_object()){	
			$id = $q->uid;
			$storyTime = $q->storyTime;
			$fadeDelay = 50;

			$gender = getData('users','gender','where id = '.$id);

			$checkUserExist = getData('users','id','where id = '.$q->uid);

			if($checkUserExist == 'noData'){
				deleteData('users_story','WHERE id = '.$q->id);
				continue;
			}

			if($custom == 'user'){
				$cDate = date('z',$storyTime);
				if (!in_array($cDate, $result)){
					$i++;
					array_push($result,$cDate);
					$storiesFilter = 'where uid = '.$id.' and storyTime >'.$storyFrom.' and deleted = 0 and review = "No"';
					$limitFilter = ' order by storyTime DESC limit 0,1';
					$story = getData('users_story','story',$storiesFilter.$limitFilter);
					$storyType = getData('users_story','storyType',$storiesFilter.$limitFilter);
					...
					$data['status'] = $status;
					$data['story'] = $story;
					$data['thumb'] = getData('users_photos','thumb','where photo = "'.$q->story.'"');
					$data['date'] = $cDate;
...
function isSub($uid1,$uid2) {
	global $mysqli;

	$query = $mysqli->prepare("SELECT COUNT(*) AS total 
                           FROM users_subscriptions 
                           WHERE u1 = ? 
                             AND u2 = ? 
                             AND days >= 1 
                             AND (FROM_UNIXTIME(time) + INTERVAL days DAY) > NOW()
                             ");
	$query->bind_param("ii", $uid1, $uid2); // Assuming u1 and u2 are integers
	$query->execute();
...
function getLastMessageMobile($uid1,$uid2){	
	...
	if ($spotlight->num_rows > 0) { 
		while($spotl = $spotlight->fetch_object()){
			if($spotl->photo > 1 ){
				$message = '<svg class="icon icon-picture"><use xlink:href="#icons-picture"></use></svg>';
			} else {
				$message = $spotl->message;			
			}
		}	
