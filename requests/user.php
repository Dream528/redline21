...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	switch (secureEncode($_POST['action'])) {
		...
		case 'landing':	
			$sm['content'] = requestPage('landing');
			echo $sm['content'];		
		break;

		case 'feed':	
			$sm['content'] = requestPage('feed');
			echo $sm['content'];		
		break;

		case 'discover':	
			$sm['content'] = requestPage('discover');
			echo $sm['content'];		
		break;

		case 'posts':	
			$sm['content'] = requestPage('posts');
			echo $sm['content'];		
		break;	

		case 'post':
			$postId = secureEncode($_POST['custom']);
			$_SESSION['post_id'] = $postId;	
			$sm['content'] = requestPage('post');
			echo $sm['content'];		
		break;					

		case 'reels':	
			$sm['content'] = requestPage('reels');
			echo $sm['content'];		
		break;			

		case 'live':	
			$sm['content'] = requestPage('live');
			echo $sm['content'];		
		break;				

		case 'settings':	
			$sm['content'] = requestPage('settings');
			echo $sm['content'];		
		break;		

		case 'profile':	
			$user = secureEncode($_POST['id']);
			if(!is_numeric($user)){
				$user =  getData('users','id','where username ="'.$user.'"');
			}		
			$creator = getData('users','creator','where id = '.$user);
			if($creator == 0){
				getUserInfo($user,1);
				$sm['content'] = requestPage('profile-me');
			} else {
				getUserInfo($user,1);
                ...
                //AUTO GENERATE POSTS
                $checkUnpostedMedia = getData('users_photos','id','where u_id ="'.$user.'" and posted = 0 order by id desc limit 1');
           
                $timeNow = time()-80000;
                if($checkUnpostedMedia != 'noData'){
                    $createPosts = getSelectedArray('id,photo,video,blur,thumb','users_photos','where u_id = "'.$user.'" and posted = 0','rand()','LIMIT 0,100'); 
                    $i=0;
                    $gallerySrc = '';
                    $galleryTotal = 0;
                    $g = 0;

                    $checkLastPostTime = getData('feed','post_time','where uid ="'.$user.'" order by post_time desc limit 1');
                    if($checkLastPostTime == 'noData'){
                    	$post_time = time();
                    } else {
                    	$post_time = $checkLastPostTime;
                    }

                    foreach ($createPosts as $f) { 
                        global $mysqli;
                        $video = $f['video'];
                        $src = $f['photo'];
                        $type = 'image';
                            ...
                            if($g == 0){
                                ...
                                if($isGallery <= 6){
                                    if($isGallery < 3){
                                        ...
                                        $galleryTotal = $isGallery;    
                                    }
                                    $gallerySrc.=$f['photo'].',';
                                    $g++;
                                    updateData('users_photos','posted',1,'WHERE id ='.$f['id']);
                                    continue;
                                } 
                            }
                            if($galleryTotal > 0 && $galleryTotal > $g){
                                $gallerySrc.=$f['photo'].',';
                                $g++;
                                updateData('users_photos','posted',1,'WHERE id ='.$f['id']);
                                continue;
                        ...
                        $rndPublishTime = rand(160,9000000);
                        $rndPublishTimeStory = rand(160,6000000);
                        $time = time()-$rndPublishTime;
                        $storyTime = time()-$rndPublishTimeStory;
                        ...
                        if($publishStory > 4){
                            ...
                            $cols = 'uid,storyTime,story,storyType,credits';
                            $vals = $user.','.$storyTime.',"'.$f['photo'].'","'.$type.'",'.$credits;
                            insertData('users_story',$cols,$vals);
                        }

                       $cols = 'uid,post_type,post_price,post_src,post_meta,post_premium,post_blur,likes,post_time,visible,post_poster';
                        $randomCaption = getData('feed_random_captions','caption','order by rand() limit 1');
                        $vals = $user.',"'.$type.'",0,"'.$src.'","'.$randomCaption.'","'.$premium.'","'.$f['blur'].'",'.$likes.','.$time.',1,"'.$poster.'"';
                        insertData('feed',$cols,$vals);
                        updateData('users_photos','posted',1,'WHERE id ='.$f['id']);

                        if($private == 1){
                            updateData('users_photos','private',1,'WHERE id ='.$f['id']);
                            if($type == 'gallery'){
                                $galleryArray = explode(',', $src);
                                foreach ($galleryArray  as $pih) { 
                                    updateData('users_photos','private',1,'WHERE photo ="'.$pih.'"');
                                }
                            }
                        }
                        $i++;
				...
				$sm['content'] = requestPage('profile');
			}
			echo $sm['content'];		
		break;			
		...
		case 'register':	
			$name = secureEncode($_POST['username']);
			$username = secureEncode($_POST['username']);
			$email = secureEncode($_POST['email']);	
			...
			if($email_check->num_rows == 1 ){
				...
				$query = "INSERT INTO users (name,email,pass,gender,age,looking,lang,join_date,bio,credits,online_day,password,ip,last_access,username,join_date_time,referral)
				VALUES ('".$username."', '".$email."','".$pswd."','0','20','2','".$lang."','".$date."','',0,0,'".$password."','".$ip."','".time()."','".$username."','".time()."','".$ref."')";					
				if ($mysqli->query($query) === TRUE) {
					$last_id = $mysqli->insert_id;
		...
		case 'delete_profile':
			...
			$mysqli->query("DELETE FROM users_subscriptions WHERE u1 = '".$uid."'");
			$mysqli->query("DELETE FROM users_subscriptions WHERE u2 = '".$uid."'");				
			...
			$mysqli->query("DELETE FROM feed WHERE uid = '".$uid."'");
		...
		case 'login':	
			...
			$user_id = getData('users','id','where email = "'.$email.'" || username = "'.$email.'"');
			if($user_id == 'noData'){
		...
		case 'recover':
			...
			$email_check = $mysqli->query("SELECT email,id,name FROM users WHERE email = '".$email."' || username = '".$email."'");	
