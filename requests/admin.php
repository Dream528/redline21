...
switch ($_POST['action']) {
	...
	case 'search_users':
		...
		$date1 = $date[0];
		$date2 = $date[1];
		$filter = 'WHERE age BETWEEN '.$age1.' AND '.$age2.' AND guest = 0';
		...
		$search = getSelectedArray('id,name,email,age,city,country,fake,admin,last_access,credits,premium,ip,online_day,verified,moderator,gender','users',$filter,$order.' desc','LIMIT 0,100');
		$i=0;
		if(isset($search)){
			foreach ($search as $value) { 
      ...
      foreach ($search as $value) { 
        ...
        $data[$i]='
            <tr class="data-search-verifications">
                ...
                <td>
                	<span class="text-muted"><small>'.$value['withdraw_details'].' <br> '.$value['withdraw_note'].'</small></span>
                </td>                                
                <td>
    ...
    if(isset($search)){
      foreach ($search as $value) { 
      	...
      	$uploadedPhoto = [];
      	$uploadedPhotoOnClick = [];

    	$media = explode(',',$value['media']);
    	for ($i=0; $i < count($media) ; $i++) { 
	    	$uploadedPhoto[$i] = '
	    		<img src="'.$media[$i].'" class="avatar-img rounded">
	    	';
	    	$uploadedPhotoOnClick[$i] = "'".$media[$i]."'"; 
    	}
		...
		$dropdown = '
      	...
      	$uEmail = getData('users','email','where id ='.$userId);
      	$nationalID = getData('users_custom_vars','c_val','where uid ='.$userId.' AND c_var ="national_id"');
      	$CPF = getData('users_custom_vars','c_val','where uid ='.$userId.' AND c_var ="cpf"');
        ...
        $data[$i]='
            <tr class="data-search-verifications">
                ...
                <td>
                	<small class="text-muted">'.$nationalID.'</small>
                </td>
                <td>
                	<small class="text-muted">'.$CPF.'</small>
                </td>                                
                <td>
	                <div class="avatar avatar-sm mr-3" onclick="showImageVerification('.$uploadedPhotoOnClick[0].')" style="margin:0 auto;cursor:pointer">
	                	'.$uploadedPhoto[0].'
	                </div>
	                <div class="avatar avatar-sm mr-3" onclick="showImageVerification('.$uploadedPhotoOnClick[1].')" style="margin:0 auto;cursor:pointer">
	                	'.$uploadedPhoto[1].'
	                </div>
	                <div class="avatar avatar-sm mr-3" onclick="showImageVerification('.$uploadedPhotoOnClick[2].')" style="margin:0 auto;cursor:pointer">
	                	'.$uploadedPhoto[2].'
	                </div>	                	                
                </td>
                <td>
	...
	case 'search_post':
		...
		$search = getArray($table,$filter,'id DESC','limit 0,250');
	    $i=0;
	    if(isset($search)){
	      foreach ($search as $value) { 
	...
	$search = getArray($table,$filter,'time DESC','LIMIT 0,100');
    $i=0;
    if(isset($search)){
      foreach ($search as $value) { 
	...
	case 'approveUserVerification':
		...
		if($approve == 1){
			$mysqli->query("UPDATE users SET verified = 1,creator = 1 where id = '".$uid."'");
