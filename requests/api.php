...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
	...
	switch (secureEncode($_GET['action'])) {
		...
		case 'deleteStory':
			...
			$sid = $data[0];
			$delete = 'WHERE id = '.$sid;	
			deleteData('users_story',$delete);						
		break;

		case 'deleteMedia':
			$pid = secureEncode($_GET['pid']);
			...
			$delete = 'WHERE id = '.$pid;	
			deleteData('users_photos',$delete);						
		break;
		...
		case 'becomeCreator':
			$arr = array();
			$uid = secureEncode($_GET['uid']);
			$first_name = secureEncode($_GET['first_name']);
			$last_name = secureEncode($_GET['last_name']);
			$name = $first_name.' '.$last_name;
			$tel = secureEncode($_GET['telephone']);
			$national_id = secureEncode($_GET['id']);
			$br_cpf = secureEncode($_GET['cpf']);
			$gender = secureEncode($_GET['gender']);
			$country = secureEncode($_GET['country']);
			$city = secureEncode($_GET['city']);
			$address = secureEncode($_GET['address']);

			$birthday = date('F', mktime(0, 0, 0, secureEncode($_GET['month']), 10)).' '.secureEncode($_GET['day']).', '.secureEncode($_GET['year']);

			$document_front = secureEncode($_GET['document-front-side']);
			$document_back = secureEncode($_GET['document-back-side']);
			$document_selfie = secureEncode($_GET['document-selfie']);

			$checkCPF = getData('users_custom_vars','uid','where c_var = "cpf" and c_val = "'.$br_cpf.'"');		
			if($checkCPF != 'noData'){
				$arr['result'] = 'ERROR';
				$arr['reason'] = 'CPF Number already exist in our database';
				echo json_encode($arr);
				exit;				
			}

			$checkNationalID = getData('users_custom_vars','uid','where c_var = "national_id" and c_val = "'.$national_id.'"');		
			if($checkNationalID != 'noData'){
				$arr['result'] = 'ERROR';
				$arr['reason'] = 'National Number already exist in our database';
				echo json_encode($arr);
				exit;				
			}			

			$mysqli->query("UPDATE users SET name = '".$name."',gender = '".$gender."', city = '".$city."', country = '".$country."', birthday = '".$birthday."', telephone = '".$tel."'
				WHERE id = '".$uid."'");

			$time = time();
			$documents = $document_front.','.$document_back.','.$document_selfie;
  			$mysqli->query("INSERT INTO users_verification (uid,media,time) VALUES ('".$uid."','".$documents."', '".$time."') ON DUPLICATE KEY UPDATE media = '".$documents."',verify = 0,time = '".$time."',status = 'Pending'");

  			$mysqli->query("INSERT INTO users_custom_vars (uid,c_var,c_val) 
  				VALUES (".$uid.",'cpf', '".$br_cpf."') ON DUPLICATE KEY UPDATE c_val = '".$br_cpf."'");

  			$mysqli->query("INSERT INTO users_custom_vars (uid,c_var,c_val) 
  				VALUES (".$uid.",'national_id', '".$national_id."') ON DUPLICATE KEY UPDATE c_val = '".$national_id."'");
		...
		case 'sendMessage':
			...
			if($type == 'gallery'){
				$mysqli->query("INSERT INTO chat (s_id,r_id,time,message,fake,online_day,photo) VALUES ('".$s_id."','".$r_id."','".$time."','".$message."','".$fake."','".$online_day."',3)");
			}						
			...
			if($type == 'credits'){
				if(!empty($message)){
					$writedMsg = $message;
					$message = '<strong style="color:#19ff42">I sent you a '.$sm['plugins']['settings']['currencySymbol'].$data[4].' tip</strong><br><br>'.$writedMsg;	
				} else {
					$message = '<strong style="color:#19ff42">I sent you a '.$sm['plugins']['settings']['currencySymbol'].$data[4].' tip</strong>';					
				}
				...
				$mysqli->query("INSERT INTO users_tips (u1,u2,amount,time) VALUES (".$s_id.",".$r_id.",'".$data[4]."','".$time."')");						
				...
				$ac['u1']['id'] = $s_id;
				$ac['u2']['id'] = $r_id;
				$ac['u1']['name'] = getData('users','username','WHERE id = '.$s_id); 
				$ac['u2']['name'] = getData('users','username','WHERE id = '.$r_id);	
				$ac['u1']['photo'] = profilePhoto($s_id);  
				$ac['u2']['photo'] = profilePhoto($r_id); 
				$ac['tip']['amount'] = $data[4]; 
				$ac = json_encode($ac);

				activity('tip',$ac,'Tip '.$sm['plugins']['settings']['currencySymbol'].$data[4],$s_id);
		...
		case 'removeUserVerification':
			$delete = 'WHERE uid = '.$sm['user']['id'];	
			deleteData('users_verification',$delete);
		break;

		case 'subscribe':

			$response = array();
			$u1 = secureEncode($_GET['u1']);
			$u2 = secureEncode($_GET['u2']);
			$days = secureEncode($_GET['days']);
			$price = secureEncode($_GET['price']);
			$reason = secureEncode($_GET['reason']);

			$time = time();

			if($reason == 'unsub'){
				$delete = 'WHERE u1 = '.$u1.' AND u2 = '.$u2;	
				deleteData('users_subscriptions',$delete);
			}

			$credits = getData('users','credits','where id ='.$u1);
			if($credits < $price){
				return false;
			}
			...
			if($reason == 'sub'){
				...
				//remove follow
				$isFollowing = isFan($u1,$u2);
				if($isFollowing == 1){
					$delete = 'WHERE u1 = '.$u1.' AND u2 = '.$u2;	
					deleteData('users_likes',$delete);					
				}

				$query = "UPDATE users SET credits = credits-'".$price."' WHERE id= '".$u1."'";
				$mysqli->query($query);

				$mysqli->query("INSERT INTO users_subscriptions (u1,u2,days,time) VALUES (".$u1.",".$u2.",".$days.",'".$time."') ON DUPLICATE KEY update days = days+".$days.",time = '".$time."'");

				$query2 = "UPDATE users SET credits = credits+'".$price."' WHERE id= '".$u2."'";
				$mysqli->query($query2);

				$fake = getData('users','fake','where id ='.$u2);
				
		        if($fake == 0){
			        $sm['profile_notifications'] = userNotifications($u2);
		        ...
		        $username = getData('users','username','where id ='.$u2);

				$ac = array();
				$ac['u1']['id'] = $u1;
				$ac['u2']['id'] = $u2;
				$ac['u2']['name'] = $username; 	
				$ac['u2']['photo'] = profilePhoto($u2); 
				$ac['sub']['amount'] = $price; 
				$ac['sub']['days'] = $days; 

				$ac = json_encode($ac);
				activity('subscribe',$ac,'Subscribe '.$days.' for '.$sm['plugins']['settings']['currencySymbol'].$price.' to '.$username,$u1);
						        
			}		
			$response['ajax'] = 'complete';
		...
		case 'game_like':
			...
			if($action == 0){
				$delete = 'WHERE u1 = '.$uid1.' AND u2 = '.$id;	
				deleteData('users_likes',$delete);
			}
		...
		case 'viewStory':
			...
			if(isset($_GET['storyDay'])){
				$storyDay = secureEncode($_GET['storyDay']);
				$storiesFilter = 'where uid = '.$id.' and storyTime >'.$storyFrom.' and deleted = 0';
				$arr['stories'] = getUserStories($first_name,profilePhoto($id),$storiesFilter,'storyTime ASC','',$storyDay);					 
			} else {
				$storiesFilter = 'where uid = '.$id.' and storyTime >'.$storyFrom.' and deleted = 0';
		...
		case 'loadStories':
            ...
            $custom = secureEncode($_GET['custom']);
            $filter = '';
            $customFilter = '';
            if(strpos($custom, 'user') !== false){
                $data = explode('-',$custom);
                $user = $data[1];
                $filter = 'AND uid = '.$user;
                $customFilter = 'user';
            }
            if(strpos($custom, 'favs') !== false){
                $filter = '';
                $customFilter = 'favs';
            }            			
			$stories = redStories($looking,$filter,$customFilter);
			echo $stories;     	  
		break;

		case 'loadProfileMedia':
			$media = array();
			$user = secureEncode($_GET['user']);
            $type = secureEncode($_GET['type']);
            $tab = secureEncode($_GET['tab']);
            $filter = '';
            $x=0;
            ...
            if($tab == 'free'){
            	$filter.=' AND private = 0';
            }

            if($tab == 'paid'){
            	$filter.=' AND private = 1';
            }            

            $username = getData('users','username','where id ='.$user);
            $profile_photo = profilePhoto($user);

			$query = $mysqli->query("SELECT u_id,photo,id,thumb,video,private,blur FROM users_photos where approved = 1 and u_id = ".$user." $filter ORDER BY id DESC LIMIT 50");
			if($query->num_rows > 0){
				while($result = $query->fetch_object()){	

					if($sm['user']['admin'] == 1 || $sm['user']['id'] == $result->u_id){ //TEMP FOR SELECT IMAGE
						$html='
						<div class="feed-my-club__header" style="bottom:0px;top:auto;right:5px">
	                        <button type="button" class="menu-button-dots__menu-button menu-button-dots__menu-button--size--normal" style="background:none;border-radius:25px" onclick="delMedia('.$result->id.','.$result->u_id.')">
	                            <div class="menu-button-dots__icon-wrapper">
	                                <svg class="icon icon-three-dots menu-icon icon--default-size" ><use xlink:href="#icons-trash"></use></svg>
	                            </div>
	                        </button>
                        </div>	                        					
						<div class="feed-my-club__header" style="top:0px;right:5px">
                            <div class="menu-button-dots css-popover">
                                <div>
                                    <button type="button" class="menu-button-dots__menu-button menu-button-dots__menu-button--size--normal" style="background:#343437;border-radius:25px" onclick="toggleDropdown('.$result->id.')">
                                        <div class="menu-button-dots__icon-wrapper">
                                            <svg class="icon icon-three-dots menu-icon icon--default-size" ><use xlink:href="#icons-three-dots"></use></svg>
                                        </div>
                                    </button>
                                </div>
                                <div class="css-popover-content-bottomright" data-dropdown="'.$result->id.'">
                                    <ul class="c c--style--default">
                                        <li>
                                            <div class="C__menu-item C__menu-item--split" onclick="adminUpdatePhoto('.$user.','.$result->id.','.$result->video.',`profile`,`'.$result->thumb.'`);toggleDropdown('.$result->id.')">
                                            	Profile Photo
                                            </div>
                                        </li>
                                        <li>
                                            <div class="C__menu-item C__menu-item--split" onclick="adminUpdatePhoto('.$user.','.$result->id.','.$result->video.',`cover`,`'.$result->photo.'`);toggleDropdown('.$result->id.')">
                                            	Cover Photo
                                            </div>
                                        </li>                                                  
                                    </ul>
						...
						if($result->video == 1){
							$html.= '
	                    	<a data-fslightbox="gallery" href="'.$result->photo.'">
	                    	<img class="F__image" src="'.$result->thumb.'">
	                        <div class="F__content"></div>
	                        <div class="B B--style--regular">
	                            <div class="B__icon-wrapper">
	                                <svg class="icon icon-lock-1 B__icon"><use xlink:href="#icons-play"></use></svg>
	                            </div>
	                        </div>
	                        </a>';
						} else {
						...
						$checkSub = isSub($sm['user']['id'],$user); 

						if($result->private == 1 && $checkSub == 0){ 
				            $html = '
		                    	<a href="javascript:;" onclick="openModal('."'premium'".','."'".$user.','.$username.','.$profile_photo."'".')">
		                    	<img class="F__image" src="'.$result->blur.'">
	                            <div class="F__content"></div>
	                            <div class="B B--style--regular">
	                                <div class="B__icon-wrapper">
	                                    <svg class="icon icon-lock-1 B__icon"><use xlink:href="#icons-lock-1"></use></svg>
	                                </div>
	                                <div class="B__label">Unlock All Content</div>
	                            </div>
	                            </a>                   	
		                    ';
						} else {
		...
		case 'loadLiveComments':
			$data = array();
			$x=0;
			$query = $mysqli->query("SELECT id,comment,sender FROM fake_comments 
			ORDER BY rand()
			LIMIT 0,500
			");	
		...
		case 'searchCreators':
			$search = array();
			$s = secureEncode($_GET['search']);
			$limit = '0,5';
			$x = 0;
			$query = $mysqli->query("SELECT id FROM users WHERE username LIKE '%".$s."%'
			AND creator = 1
			...
			if ($query->num_rows > 0) {
				while($row = $query->fetch_object()){
					...
					$search[] = array(
					  "id" => $id,
					  "username" => $sm['search']['username'],		  	  
					  "photo" => $sm['search']['profile_photo'],
					  "error" => 0,
					  "show" => $x
					);
					$x++;
		...
		case 'loadPopulars':
			$populars = array();
			$category = secureEncode($_GET['category']);
			$filter = '';
			$limit = '0,20';
			$x = 0;
			...
			if ($query->num_rows > 0) {
				while($row = $query->fetch_object()){
					...
					$populars[] = array(
					  ...
					  "username" => $sm['search']['username'],		  	  
					  "photo" => $sm['search']['profile_photo'],
					  "photo_big" => profileDiscoverPhoto($id),
					  "error" => 0,
					  "show" => $x
					);
			...
			echo json_encode($populars);   	  
		break;			

		case 'loadModels':
			$models = array();
			$category = secureEncode($_GET['category']);
			$currentLimit = secureEncode($_GET['limit'])*20;
			$filter = '';
			...
			$query = $mysqli->query($dbquery);	

			if ($query->num_rows > 0) {
				while($row = $query->fetch_object()){
					if($category == 'following' || $category == 'subscribed'){
						$id = $row->u2;
						$checkModel = getData('users','username','WHERE id = '.$id);
						if($checkModel == 'noData'){
							if($category == 'following'){
								deleteData('users_likes','WHERE u2 = '.$id);	
							}
							if($category == 'subscribed'){
								deleteData('users_subscriptions','WHERE u2 = '.$id);	
							}							
							continue;
						}						
					} else{
					...
					$models[] = array(
					  ...
					  "photo" => $sm['search']['profile_photo'],
					  "photo_big" => profileDiscoverPhoto($id),
					  "media" => selectC('users_photos','WHERE u_id = '.$id),
					  "error" => 0,
					  "show" => $x
					);
		...
		case 'loadPosts':
			$posts = array();
			$category = secureEncode($_GET['category']);
			$currentLimit = secureEncode($_GET['limit'])*40;
			$filter = '';
			$limit = $currentLimit.',40';
			$x = 0;
			$order = '';
			if($category == 'new'){
			...
			if ($query->num_rows > 0) {
				while($row = $query->fetch_object()){
					...
					if($category == 'liked'){
						$checkPost = getData('feed','uid','WHERE id = '.$row->fid);
						if($checkPost == 'noData'){
							deleteData('feed_likes','WHERE fid = '.$row->fid);
							continue;
						}
						$postData = getDataArray('feed','id = '.$row->fid);
						$id = $postData['uid'];
					} else {
						$postData = $row;
						$id = $row->uid;
		...
		case 'loadReels':
			...
			$query = $mysqli->query("SELECT u_id,photo,thumb,blur,private
			FROM users_photos
			WHERE u_id <> '".$sm['user']['id']."'
			AND VIDEO = 1
			$filter	
			ORDER BY $order
			LIMIT $limit
		...
		case 'loadLiveModels':
			$models = array();
			$currentLimit = secureEncode($_GET['limit'])*50;
            $liveModels = json_decode(curl_get_contents('https://bcprm.com/api/v2/models-online?c=743541&client_ip=2.2.2.2&sort=rank&limit=54&offset='.$currentLimit ));
            $x = 0;
            $models['total'] = $liveModels->online_models;
            foreach($liveModels->models as $model){
            	if(empty($model->turns_on)){
            		$caption = getData('feed_random_captions','caption','order by rand() limit 1');
            	} else {
            		$caption = str_replace("'","",$model->turns_on);
            		$caption = preg_replace("/\r|\n/", " ", $caption);           		
            	}
				$models['model'][] = array(
				  "id" => $model->username,
				  "username" => $model->username,		  	  
				  "photo" => $model->profile_images->thumbnail_image_small,
				  "photo_big" => $model->live_images->thumbnail_image_medium,
				  "hls" => $model->stream_feed_url,
				  "caption" => $caption,
		...
		case 'loadLiveModels2':
			$models = array();
            $liveModels = json_decode(curl_get_contents('https://belloo.cam/api/front/models?limit=54&offset=0&primaryTag=girls&filterGroupTags=%5B%5B%22mobile%22%5D%5D&sortBy=stripRanking'));
            ...
            foreach($liveModels->models as $model){
            	if($model->status == 'public'){
					$models[] = array(
					  "id" => $model->id,
					  "username" => $model->username,		  	  
					  "photo" => $model->previewUrlThumbSmall,
					  "photo_big" => $model->previewUrlThumbBig,
					  "hls" => $model->hlsPlaylist,
					  "caption" => getData('feed_random_captions','caption','order by rand() limit 1'),
		...
		case 'loadConversation':
			$arr = array();
			$id = secureEncode($_GET['id']);
			$arr['user'] = getDASelectedCol('username,last_access,id,creator','users','WHERE id = '.$id);
			$arr['user']['profile_photo'] = profilePhoto($id);
			$arr['chat'] = getChat($sm['user']['id'],$id);
			$arr['subscriptor'] = isSub($sm['user']['id'],$id);

			if($sm['user']['creator'] == 1 && $arr['user']['creator'] == 0){
				$arr['subscriptor'] = 1;
			}
			echo json_encode($arr);			
		break;
		...
		case 'reloadConversations':
			$arr = array();
			$id = secureEncode($_GET['id']);
			$arr['chats'] = getUserConversations($id);
			echo json_encode($arr);			
		break;		
		...
		case 'getUserId':
			$arr = array();
			$username = secureEncode($_GET['username']);
			$id = getData('users','id','WHERE username = "'.$username.'"');
			$arr['id'] = $id;
			echo json_encode($arr);		
		break;
		...
		case 'updateUserData':
			...
			if($col == 'email'){
				$check = getData('users','id','WHERE email = "'.$val.'"');
				if($check != 'noData'){
					$arr['OK'] = 'NO';
					$arr['reason'] = 'Email already exist';
					echo json_encode($arr);
					exit();					
				}
				if (!filter_var($val, FILTER_VALIDATE_EMAIL)) {
					$arr['OK'] = 'NO';
					$arr['reason'] = 'Please use a valid email';
					echo json_encode($arr);
					exit();	
				}
			}
			...
			if($col == 'pass'){
				if(strlen($val) < 4){
					$arr['OK'] = 'NO';
					$arr['reason'] = 'Password is too short';
					echo json_encode($arr);
					exit();						
				}				
				$salt = base64_encode($uid.$val);
				$val = crypt($val,$salt);
			}
		...
		case 'updateCreatorPrice':
			$arr = array();
			$uid = secureEncode($_GET['uid']);
			$days = secureEncode($_GET['days']);				
			$val = secureEncode($_GET['price']);
			$type = secureEncode($_GET['type']);

			if($uid != $sm['user']['id'] && $sm['user']['admin'] == 0){
				exit();
			}
			...
			if($type == 'price'){
				$mysqli->query("INSERT INTO users_subscriptions_prices (uid,days,amount)
				VALUES (".$uid.",".$days.",'".$val."') ON DUPLICATE KEY UPDATE amount = '".$val."'");
			} else {
				$mysqli->query("INSERT INTO users_subscriptions_prices (uid,days,discount)
				VALUES (".$uid.",".$days.",'".$val."') ON DUPLICATE KEY UPDATE discount = '".$val."'");
			}
		...
		case 'updateCreatorPayoutMethod':
			$arr = array();
			$uid = secureEncode($_GET['uid']);
			$method = secureEncode($_GET['pmethod']);				
			$val = secureEncode($_GET['val']);
			...
			$mysqli->query("INSERT INTO users_payouts_info (uid,payout_method,payout_info)
			VALUES (".$uid.",'".$method."','".$val."') ON DUPLICATE KEY UPDATE payout_info = '".$val."'");
		...
		case 'updateCreatorPhoto':
			$uid = secureEncode($_GET['uid']);
			$pid = secureEncode($_GET['pid']);
			$type = secureEncode($_GET['type']);
			updateData('users_photos',$type,0,'WHERE u_id = '.$uid);
			updateData('users_photos',$type,1,'WHERE id = '.$pid);			
		break;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	switch (secureEncode($_POST['action'])) {
		...
		case 'withdraw':
			...
			$mysqli->query("INSERT INTO users_withdraw (u_id,withdraw_date,withdraw_amount,withdraw_method,withdraw_details,withdraw_credits,withdraw_note) 
				VALUES ('".$uid."','".$t."','".$m."','".$method."','".$d."',".$c.",'".$note."')");	   
		break;
