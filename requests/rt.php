...
require '../assets/includes/pusher/autoload.php';
require('../assets/includes/custom/app_core.php');

date_default_timezone_set(pluginsData('settings','timezone'));
$options = array(
'cluster' => pusherData('cluster')
);
$rt = new Pusher\Pusher(
    pusherData('key'),
    pusherData('secret'),
    pusherData('id'),
    $options
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    switch ($_GET['action']) {
        case 'message':
            ...
            $arr['message'] = clearMessageBR($message);
            $arr['id'] = $s_id;
            $arr['action'] = 'message';
            $arr['type'] = 'type';
            $arr['icon'] = $photo;
            ...
            $notiData['clean_msg'] = clearMessageBR($message);
